...
class TodoForm(forms.Form):
    text = forms.CharField(
        ...
        # when we want to change default error message text,
        # we should check the default codes for filed type
        error_messages={
...
class TodoCreateFrom(forms.ModelForm):
    class Meta:
        ...
        error_messages = {
            'text': {
                'max_length': 'The `text` is to long !'
            }
        }
...
class PersonCreateForm(forms.ModelForm):
    ...
    def clean_profile_image(self):
        """
        Change the filename of the `profile_image` to something autogenerated
        :return: profile_image: str
        """

        profile_image = self.cleaned_data['profile_image']
        # `f'{str(uuid.uuid4())}.jpg'` will generate something like this:
        # `persons/18584cd7-2d29-4bae-a38b-b46fb4af9a23.jpg`
        profile_image.name = f'{str(uuid.uuid4())}.jpg'

        return profile_image

    def clean(self):
        """
        If we want filename of the `profile_image` to be based on the `Person` name,
        we should use `clean` method
        :return: profile_image: str
        """

        super().clean()  # After this, all values are in `cleaned data`
        profile_image = self.cleaned_data['profile_image']
        profile_image = f'{self.cleaned_data['name']}.jpg'
